...
(async ()=>{
    ...
    function crunchNumbers(courseData, assignments, submissions, users) {
        ...
        let groupWeightedPoints = {};
        possibleAssignments.forEach((a) => {
            if (!(a.assignmentGroup.name in groupWeightedPoints)) {
                groupWeightedPoints[a.assignmentGroup.name] = 0;
            }
            groupWeightedPoints[a.assignmentGroup.name] += a.pointsPossible;
        });
        console.log("Group Weighted Points", groupWeightedPoints);
        let runningSum = 0;
        ...
        possibleAssignments.forEach(a => {
            ...
            console.log(a)
            const k = a.pointsPossible * a.assignmentGroup.groupWeight / groupWeightedPoints[a.assignmentGroup.name];
            runningSum += k;
            if (dueAt in seenDates) {
                points.possible.values[seenDates[dueAt]].y = runningSum;
                points.possible.values[seenDates[dueAt]].k.push(k);
                points.possible.values[seenDates[dueAt]].description += ", " + a.name;
            } else {
                seenDates[dueAt] = points.possible.values.length;
                points.possible.values.push({x: dueAt, y: runningSum, "k": [k], description: a.name});
            }
        });
        ...
        Object.values(submissions).map(sub=>
            (sub.submittedAt != null ? sub : {...sub, submittedAt: sub.score ? sub.assignment.dueAt : crunched.endAt})
        ).sort(submissionByDate).forEach(sub => {
            const dueAt = moment(sub.submittedAt).format('YYYY-MM-DD');
            studentGrades[sub.user.id] += sub.score * sub.assignment.assignmentGroup.groupWeight /groupWeightedPoints[sub.assignment.assignmentGroup.name];
            scoresOnDate[dueAt] = {...studentGrades};
            namesOnDate[dueAt] = sub.assignment.name;
        });
        crunched.points.median.values = Object.entries(scoresOnDate).map(([dueAt, grades]) => {
            return {x: dueAt, y: median(Object.values(grades)), description: Object.values(grades).length + " submissions"};
        });
        ...
        if (student) {
            ...
            studentSubmissions = Object.values(submissions).filter(
                ...
                (sub.submittedAt != null ? sub :
                     {...sub, submittedAt: sub.score ? sub.assignment.dueAt : crunched.endAt})
            ...
            points.student.values = scored.map(sub => {
                runningSum += (sub.score || 0) *  sub.assignment.assignmentGroup.groupWeight/ groupWeightedPoints[sub.assignment.assignmentGroup.name];
                return {x: sub.submittedAt, y: runningSum, description: sub.assignment.name};
            });
            points.student.labels = scored.map(sub =>
                sub.assignment.name);
            points.unscored.values = [
                ...unscored.map(sub => {
                    runningSum += (sub.assignment.pointsPossible || 0) * sub.assignment.assignmentGroup.groupWeight/ groupWeightedPoints[sub.assignment.assignmentGroup.name];
                    return {x: sub.assignment.dueAt || crunched.endAt, y: runningSum, description: sub.assignment.name};
                }),
                ...(Object.values(assignments).filter(a => {
                    return !(a.id in seenAssignments);
                }).map(a => {
                    runningSum += a.pointsPossible *  a.assignmentGroup.groupWeight/ groupWeightedPoints[a.assignmentGroup.name];
                    return {x: a.dueAt || crunched.endAt, y: runningSum, description: a.name};
                }))
            ];
        }
